...
module.exports = {
	...
	tabCreated: (req, res, next) => {
    	const resourceIsBlocked = (request) => {
    		const { host, pathname } = url.parse(request.url);
    		const basename = path.basename(pathname);
    		return this.blockedFiles.some((filePattern) => minimatch(basename, filePattern)) ||
				this.blockedDomains.some((domainPattern) => minimatch(host, domainPattern));
		};
        req.prerender.tab.Network.setRequestInterception({enabled: true, patterns: [ {urlPattern: '*'}]}).then(() => {
            req.prerender.tab.Network.requestIntercepted(({interceptionId, request}) => {
                const blocked = resourceIsBlocked(request);
