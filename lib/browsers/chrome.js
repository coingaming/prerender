    ...
    return new Promise((resolve, reject) => {
        ...
        Promise.all([
            ...
            Network.loadingFailed((params) => {
                ...
                if (tab.prerender.requests[params.requestId]) {
                    tab.prerender.numRequestsInFlight--;
                    if (tab.prerender.logRequests || this.options.logRequests) util.log('-', tab.prerender.numRequestsInFlight, tab.prerender.requests[params.requestId], params.errorText);
                    delete tab.prerender.requests[params.requestId];
                }
            });

            Log.entryAdded((params) => {
                if (tab.prerender.logRequests || this.options.logRequests) util.log('CONSOLE LOG', params.entry);
            });
        ...
        Page.enable()
            .then(() => {
                ...
                setTimeout(() => {
                    if (!finished) {
                        finished = true;
                        util.log('Page timed out for ' + tab.prerender.url + 'Pending requests: ' + Object.keys(tab.prerender.requests).reduce(function (accumulator, requestKey) {
                            return accumulator + tab.prerender.requests[requestKey] + ' | ';
                        }, ''));
                        resolve();
                    }
                }, pageLoadTimeout);
